...
function ProductDetail() {
 ...
 const [sizevalue, setSizValue] = useState("No Size")
 const [qtyvalue, setQtyValue] = useState(1)
 const [reviews, setReviews] = useState([])
 const [creatReviews, setCreateReviews] = useState({
     review: "", rating: 0, product_id: product.id, user_id:0
 })
 ...
 const handlerSizeButton = (event) =>{
    const sizeNameInput = event.target.closest('.size_button').parentNode.querySelector(".size_value")
    setSizValue(sizeNameInput.value)
     Toast().fire({
          icon : "success",
          title : "Size chosed",
        })
 ...
 const handleAddToCart = async (event) => {
    ...
    try {
            ...
            formdata.append("shipping", product.shipping_amount)
...
const fetchReviewData = async () => {
    if (product !== null){          
       await apiInstance.get(`reviews/${product?.pid}/`).then ((res) => {
           setReviews(res.data);
      })
    }
}
...
try {
        ...
        formdata.append('user_id', userData.sub)
        formdata.append('product_id', product?.id)
        formdata.append('rating' ,creatReviews.rating)
        formdata.append('review' ,creatReviews.review)
                        ...
                        <div className="col-md-6 mb-4 mb-md-0">
                            ...
                            <div>
                                    ...
                                    <ul className="mb-3 d-flex p-0" style={{ listStyle: "none" }}>
                                        ...
                                        <li style={{ marginLeft: 10, fontSize: 13 }}>
                                            <a href="" className="text-decoration-none">
                                                <strong className="me-2">{product.rating}</strong>({product.rating_count} reviews)
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                                ...
                                <form action="">
                                    <div className="row flex-column">
                                        ...
                                        <div className="col-md-6 mb-4">
                                            ...
                                            <div className='d-flex'>
                                                ...
                                                <button key={index} type="button" onClick={handlerSizeButton} className="btn btn-secondary m-1 size_button">{s.name}</button>   
                                                </div> 
                                                 ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="col-md-6 mb-4">
                                            <div className="form-outline">
                                                <label className="form-label" htmlFor="typeNumber"><b>Color:</b> <span></span></label>
                                            </div>
                                            <div className='d-flex'>
                                             {color?.map((c, index) => (
                                             <div className='d-flex' key={index} >
                                               <input type="hidden" className='color_name' value={c.name} name="" id=""/>
                                               <button  className='btn p-3 m-1 color_button' type="button" onClick={handleColorButtonClick} style={{ background: `${c.name}` }}></button>
                                             </div>
                                              ))} 
                                            </div>
                                            <hr />
                                    ...
                                    onClick={() => addToWishList(product?.pid)} data-mdb-toggle="tooltip" title="Add to wishlist">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        {product?.color?.map((c, index) => (
                                            <>
                                             <button  className='btn p-3 m-1 color_button' type="button" style={{ background: `${c.name}` }}></button>
                                            </>
                                        ))}  
                                    </tr>
                                    <tr>
                                        ...
                                        {product?.size?.map((p, index) => (  
                                            <div className="btn btn-secondary m-1 size_button" key={index}>{p.name}</div>        
                                        ))}  
                                    </tr>
                                    <tr>
                                        <th className="ps-0 w-25" scope="row">
                            ...
                            <div className="row">
                                ...
                                <div className="col-md-6">
                                    ...
                                    <div className="card mb-3" key={index} >                                           
                                            <div className="row g-0 border p-2" >
