...
import { SERVER_URL, PAYPAL_CLENT_ID } from '../../utils/constantts'
...
function CheckOut() {
    ...
    const applyCoupon = async () => {
        ...
        formdata.append("coupon_code", coponCode)
                            ...
                            <section className="shadow-4 p-4 rounded-5 mb-4">
                                ...
                                <div className="shadow p-3 d-flex mt-4 mb-4">
                                    <input  name="couponCode" type="text" 
                                     ...
                                     className='form-control' style={{ border: "dashed 1px gray" }} placeholder='Enter Coupon Code' id="" />
                                    <button  onClick={applyCoupon} className='btn btn-success ms-1'><i className='fas fa-check-circle'></i></button>
                                </div>
                                {paymentLoading === false &&

                                    <form action={`http://127.0.0.1:8000/api/v1/stripe-checkout/${order?.oid}/`} method='POST'>
                                        <button type="submit" onClick={payWithStripe}
                                         className="btn btn-primary btn-rounded w-100 mt-2" style={{ backgroundColor: "#635BFF" }}>
                                            <i className='fas fa-credit-card '></i>Pay Now (Stripe)</button>
                                    </form>
                                ...
                                {paymentLoading === true &&  
                                    ...
                                    <form action={`http://127.0.0.1:8000/api/v1/stripe-checkout/${order?.oid}/`} method='POST'>
                                        <button type="submit" onClick={payWithStripe} disabled
                                        className="btn btn-primary btn-rounded w-100 mt-2" style={{ backgroundColor: "#635BFF" }}>Processing...
